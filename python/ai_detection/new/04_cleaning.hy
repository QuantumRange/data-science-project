...
(defn main []
  (setv files (sorted (list (.glob (Path "/mnt/Fast2T/data/new/output/") "*.parquet"))))
  (setv files (list (filter (fn [file] (not (.exists (/ (Path "/mnt/Fast2T/data/new/clean_data/") (. file name))))) files)))
  (for [file (tqdm files)]
   (setv df (pipeline (pl.read_parquet file)))
   (.write_parquet df (/ (Path "/mnt/Fast2T/data/new/clean_data/") (. file name)))))
